...
dlist_t *DListCreate(void)
{
	...
	dummy_head = (dnode_t *)malloc(sizeof(dnode_t));	
	dummy_tail = (dnode_t *)malloc(sizeof(dnode_t));
...
ditr_t DListInsertBefore(dlist_t *list, ditr_t where, void *data)
{
	...
	where_prev_node = where_node->prev;
...
ditr_t DListRemove(ditr_t what)
{
	...
	node_next = what_node->next;
	nude_prev = what_node->prev;	
...
{
	size_t size_list = 0;
	dnode_t *count_size = NULL;
	dnode_t *end = NULL;
	
	assert(NULL != list);
	
	count_size = list->head->next;
	end = list->tail;
