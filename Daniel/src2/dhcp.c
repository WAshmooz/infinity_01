...
status_t DhcpAllocateIp(dhcp_t *dhcp, ip_adr_t preffered_ip, ip_adr_t *result)
{
	...
	size_t i = 0;
	size_t index_of_bit = (IP_SIZE - tree_hight + 1);
	...
	for (i = 0; i < tree_hight; i++)
	{
		...
		if (FREE == current->node[bit]->is_full)
		{
			parnet_node = &current;
			StackPush(stack_parents, parnet_node);
			current = current->node[bit];			
		...
		/*if next node full, go back to parent using stack*/
		if (FULL == current->node[bit]->is_full)
		{
			if (true == StackIsEmpty(stack_parents))
			{
				/*TODO update result*/
				return status;
			}
...
static void PrintBTreeFromVector(vector_t *vec, print_element_t printFunc)
{
	int i = 0, j = 0, level = 0;
	int curIdx = 0;
