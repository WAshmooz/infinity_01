...
struct arguments  //size, *device, verbose
{
...
{
    /*unsigned long long [size] , char [*device], int [verbose]*/
  struct arguments arguments = {  //set verbose to 0
      .verbose = 0,
  };
  argp_parse(&argp, argc, argv, 0, 0, &arguments);

  struct buse_operations aop = { //xmp_read, xmp_write.. xmp_trim, size(arguments)
      .read = xmp_read,
      .write = xmp_write,
      .disc = xmp_disc,
      .flush = xmp_flush,
  ...
  if (data == NULL){ //EXIT_FAILURE - failed alloc
