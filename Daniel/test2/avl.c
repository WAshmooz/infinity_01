...
static avl_node_t *Balance_Node(avl_node_t *node, is_before_t is_before_func)
{
	...
	printf("Balance_Node %d, %d %d %d\n", *(int*)node->data, balance_fac, left_height, right_height);
...
static void AvlPrintGo(avl_node_t *root)
{
	queue qu;
	avl_node_t *node;
	int half = 128;
	...
	int nodes_on_level;
	int safeStop = 10;
	qu.top = qu.bottom = qu.size = qu.realVal = 0;
	for (i = 0; i < SIZE; ++i) {
		qu.arr[i] = NULL;
	}
	
	Queue_push_back(&qu, root);
	while (--safeStop && 0 < qu.realVal) {
		nodes_on_level = qu.size;
		printf("%*s", half/2, "");
		for (i = 0; i < nodes_on_level; ++i)
		{
			node = (avl_node_t*)Queue_pop_front(&qu);
			
			
			if (i % 2) {
				if (NULL == node) printf("\\  ");
				else              printf("\\%02d-%d", *(int*)(node->data), node->height);
			}
			else {
				if (NULL == node) printf("  /");
				else              printf("%2d-%d/", *(int*)(node->data), node->height);
			}
			printf("%*s", half-4, "");
			
			
			if (node) {
				Queue_push_back(&qu, node->children[LEFT]);
				Queue_push_back(&qu, node->children[RIGHT]);
			}
			else  {
				Queue_push_back(&qu, NULL);
				Queue_push_back(&qu, NULL);
			}
		}
		printf("\n");
