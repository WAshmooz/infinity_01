...
/* WES */
int actionPrint(void *data, void *user_params)
{
	hash_elem_t *hash_elem = (hash_elem_t*)data;
	if (data)
		/* printf("%d, ", *(int*)data); */
		printf("%s %d, ", (char*)hash_elem->key, *(int*)hash_elem->val);
}

void HashPrint(hash_t *hash)
{
	int i = 0;
	dlist_t *list;
	...
	for (i = 0; i < hash->table_size; ++i) {
		printf("  [%2d]: ", i);
		list = hash->table[i];
		DListForEach(DListIterBegin(list), DListIterEnd(list), actionPrint, NULL);
		printf("\n");
	}
}
